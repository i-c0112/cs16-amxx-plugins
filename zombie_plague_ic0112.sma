...
#include <nadeammoapi>
 ...
 #pragma reqlib noroundend
 #if !defined AMXMODX_NOAUTOLOAD
  #pragma loadlib noroundend
 #endif
#else
 #pragma library noroundend
#endif
...
new const sound_first_zombie[][] = { "zombie_plague/first_zombie1.wav", "zombie_plague/first_zombie2.wav" } //第一隻喪屍出現時的聲音
//回合開始倒數計時的聲音(英文數字1~10)
//可以自行設定要使用男聲還是女聲,看要使用那一種聲音只要去掉該項 new const sound_countdown 前面的 // 字符,並在另一項前面加上 // 字符即可.
//男聲
new const sound_countdown[20][] = { "vox/one.wav", "vox/two.wav", "vox/three.wav", "vox/four.wav", "vox/five.wav", "vox/six.wav", "vox/seven.wav", "vox/eight.wav", "vox/nine.wav", "vox/ten.wav",
										"", "", "", "", "" , "", "", "", "", "vox/zombi_start.wav" }
//女聲
...
new const MAXBPAMMO[] = { -1, 104, -1, 180, 1, 64, 1, 200, 180, 1, 240, 200, 200, 180, 180, 400, 200, 240,
			60, 240, 200, 64, 180, 240, 180, 2, 70, 180, 180, -1, 200 }
...
new const BUYAMMO[] = { -1, 26, -1, 60, -1, 16, -1, 40, 60, -1, 120, 100, 24, 60, 60, 60, 24, 60,
			20, 60, 60, 16, 60, 60, 60, -1, 14, 60, 60, -1, 100 }
...
new const WEAPONNAMES[][] = { "", "P228 手槍", "", "Scout 輕型狙擊槍", "", "XM1014 連發散彈槍", "", "MAC10 小型衝鋒槍", "AUG 近狙型步槍",
			"", "Dual Elite 雙槍", "Fiveseven 手槍", "UMP45 衝鋒槍", "SIG550 連發狙擊槍", "Galil 步槍", "Famas 步槍",
			"USP 手槍", "Glock 手槍", "AWP 麥格農狙擊槍", "MP5 衝鋒槍", "M249 班用輕機槍",
			"M3 散彈槍", "M4A1 步槍", "TMP 衝鋒槍", "G3SG1 連發狙擊槍", "", "Deagle 手槍",
			"SIG552 近狙型步槍", "AK47 步槍", "", "P90 衝鋒槍" }
...
const PEV_NADE_TYPE = PEV_NADEAMMOID
//const NADE_TYPE_INFECTION = 1111	// this feature is now using NAA support
...
new g_updatedata[33][4] // updatestatus / frags / deaths / armor
new g_headshot[33] // killed by headshot
...
new g_restartround // used when last human infected
...
new g_time_countdown // 回合開始倒數計時用的變數
new g_playerscount	//回合開始時紀錄人數,用來計算第一隻殭屍血量
...
cvar_fireprice, cvar_firelimit, cvar_frostprice, cvar_frostlimit, cvar_flareprice, cvar_flarelimit, cvar_infbombprice
...
// NAA vars
new g_nade_fire, g_nade_frost, g_nade_flare, g_nade_infect
...
public plugin_natives()
{
	...
	register_native("zp_get_round_player_count", "native_get_round_player_count", 1)
...
public plugin_precache()
{
	...
	for (i = 0; i < sizeof sound_first_zombie; i++)
		engfunc(EngFunc_PrecacheSound, sound_first_zombie[i])

	// 回合開始倒數計時的聲音 (from Half-Life vox)
...
public plugin_init()
{
	...
	// Set cvar from noroundend module to support last human infection
	...
	new buffer[11]; num_to_str(g_extra_costs2[EXTRA_INFBOMB], buffer, charsmax(buffer));
	cvar_infbombprice = register_cvar("zp_extra_infbomb_price", buffer)
	...
	cvar_fireprice = register_cvar("zp_fire_price", "8")
	cvar_firelimit = register_cvar("zp_fire_limit", "15")
	cvar_frostgrenades = register_cvar("zp_frost_grenades", "1")
	cvar_freezeduration = register_cvar("zp_frost_duration", "3")
	cvar_frostprice = register_cvar("zp_frost_price", "5")
	cvar_frostlimit = register_cvar("zp_frost_limit", "5")
	...
	cvar_flareprice = register_cvar("zp_flare_price", "3")
	cvar_flarelimit = register_cvar("zp_flare_limit", "20")
	...
	// Human get score when game is commencing
	g_scorehumans = -1
...
{
	// Last param, PCVAR_teams, is ignored because grenades access should always be evaluated by ACCEPT_SPC in zombieplague mod
	g_nade_fire = RegisterNade("Fire Nade", GRENADE_EXPLOSIVE, ARMOURY_MODEL_NONE, cvar_fireprice, cvar_firelimit)
	g_nade_frost = RegisterNade("Frost Nade", GRENADE_FLASHBANG, ARMOURY_MODEL_NONE, cvar_frostprice, cvar_frostlimit)
	g_nade_flare = RegisterNade("Flare Nade", GRENADE_SMOKEGREN, ARMOURY_MODEL_NONE, cvar_flareprice, cvar_flarelimit)
	g_nade_infect = RegisterNade("Infection Bomb", GRENADE_EXPLOSIVE, ARMOURY_MODEL_NONE, cvar_infbombprice, cvar_infbomblimit)
}
...
public event_round_start()
{
	...
	g_time_countdown = floatround(get_pcvar_float(cvar_warmup))
	remove_task(TASK_MAKEZOMBIE)
	set_task(6.0, "make_zombie_task", TASK_MAKEZOMBIE)

	// Game commencing now wont restart round because of module "noroundend"
	if (g_scorehumans == -1) server_cmd("sv_restart 3")
}
...
public fw_PlayerSpawn_Post(id)
{
	...
	// Update datas cached when round restarting
	if (g_updatedata[id][0] == 1)
	{
		UpdateFrags(id, id, g_updatedata[id][1], g_updatedata[id][2], 1)
		set_pev(id, pev_armorvalue, float(g_updatedata[id][3]))
	}
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	// Killed by headshot damage
	if (get_pdata_int(victim, OFFSET_LAST_HIT_GROUP, OFFSET_LINUX) == HIT_HEAD)
	...
	// Cache frags/deaths when round is being restarted manually
	if (g_updatedata[victim][0] == 1)
	{
		g_updatedata[victim][1] = pev(victim, pev_frags)
		g_updatedata[victim][2] = cs_get_user_deaths(victim)
	}

	if (is_user_valid_connected(attacker) && g_updatedata[attacker][0] == 1)
	{
		g_updatedata[attacker][1] = pev(attacker, pev_frags)
		g_updatedata[attacker][2] = cs_get_user_deaths(attacker)
	}
	...
	if (get_pcvar_num(cvar_deathmatch))
	{
		...
		if ((g_zombie[victim] && !g_nemesis[victim] && (!get_pcvar_num(cvar_respawnzomb) || g_headshot[victim])) || (!g_zombie[victim] && !g_survivor[victim] && !get_pcvar_num(cvar_respawnhum)) || (g_nemesis[victim] && (!get_pcvar_num(cvar_respawnnem) || g_headshot[victim])) || (g_survivor[victim] && !get_pcvar_num(cvar_respawnsurv)))
		{
			// mark all unrespawnable players to make sure the round will restart
			g_headshot[victim] = true
			return;
		}
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	else if (g_nemesis[victim]) return HAM_IGNORED;
...
{
	// Attacker is human...
	if (is_user_valid_connected(attacker) && !g_zombie[attacker])
	{
		// Reward ammo packs
...
// Nade Ammo API support

// NAA Switch Ammo Type Forward (also called after CurNade Forward)
public fw_NAA_ChangeAmmo(id, NadeType:nade_type, ammo_to)
{
	switch (nade_type)
	{
		case GRENADE_EXPLOSIVE: // Infection bomb or fire grenade
		{
			if (ammo_to == g_nade_infect)
				set_pev(id, pev_viewmodel2, model_grenade_infect)
			else if (ammo_to == g_nade_fire)
				set_pev(id, pev_viewmodel2, model_grenade_fire)
		}
		case GRENADE_FLASHBANG: // Frost grenade
		{
			if (ammo_to == g_nade_frost)
				set_pev(id, pev_viewmodel2, model_grenade_frost)
		}
		case GRENADE_SMOKEGREN: // Flare grenade
		{
			if (ammo_to == g_nade_flare)
				set_pev(id, pev_viewmodel2, model_grenade_flare)
		}
	}
}

// NAA buy menu display, pickup Forward
public fw_NAA_CanPickup(id, NadeType:nade_type, ammo_type, PickupType:pickuptype)
{
	switch(nade_type)
	{
		case GRENADE_EXPLOSIVE:
		{
			if (ammo_type == g_nade_fire)
			{
				if (!g_zombie[id] && !g_survivor[id])
					return 1;

				if (pickuptype == PICK_BUY)
					SetReason("%L", id, "CMD_HUMAN_ONLY")
			}
			else if (ammo_type == g_nade_infect)
			{
				if (g_zombie[id] && !g_nemesis[id])
					return 1;

				if (pickuptype == PICK_BUY)
					SetReason("%L", id, "CMD_ZOMBIE_ONLY")
			}
		}
		case GRENADE_FLASHBANG:
		{
			if (ammo_type == g_nade_frost)
			{
				if (!g_zombie[id] && !g_survivor[id])
					return 1;
		...
		case GRENADE_SMOKEGREN:
		{
			if (ammo_type == g_nade_flare)
			{
				if (!g_zombie[id] && !g_survivor[id])
					return 1;
	...
	// Forward returns biggest value, so we only return 1 when available
	return 0;
}

// NAA Grenade Throw Forward
public fw_NAA_NadeThrow(id, ent, NadeType:nade_type, ammo_type)
{
	...
	if (g_zombie[id])
	{
		if (nade_type == GRENADE_EXPLOSIVE && get_pcvar_num(cvar_extrainfbomb)) // Infection Bomb
		{
			// Give it a glow
			fm_set_rendering(ent, kRenderFxGlowShell, 0, 200, 0, kRenderNormal, 16);
			...
			set_pev(ent, PEV_NADE_TYPE, g_nade_infect)
		}
	}
	else if (nade_type == GRENADE_EXPLOSIVE && get_pcvar_num(cvar_firegrenades)) // Napalm Grenade
	{
		// Give it a glow
		...
		// Set grenade type on the thrown grenade entity
		set_pev(ent, PEV_NADE_TYPE, g_nade_fire)
	}
	else if (nade_type == GRENADE_FLASHBANG && get_pcvar_num(cvar_frostgrenades)) // Frost Grenade
	{
		// Give it a glow
		...
		// Set grenade type on the thrown grenade entity
		set_pev(ent, PEV_NADE_TYPE, g_nade_frost)
	}
	else if (nade_type == GRENADE_FLASHBANG && get_pcvar_num(cvar_flaregrenades)) // Flare
	{
		// Build flare's color
		static rgb[3]
		...
		// Set grenade type on the thrown grenade entity
		set_pev(ent, PEV_NADE_TYPE, g_nade_flare)
...
public fw_ThinkGrenade(entity)
{
	...
	{
		case g_nade_infect: // Infection Bomb
		{
			infection_explode(entity)
			return HAM_SUPERCEDE;
		}
		case g_nade_fire: // Napalm Grenade
		{
			fire_explode(entity)
			return HAM_SUPERCEDE;
		}
		case g_nade_frost: // Frost Grenade
		{
			frost_explode(entity)
			return HAM_SUPERCEDE;
		}
		case g_nadeflare: // Flare
		{
			// Get its duration
			static duration
	...
	*/
	static nadeAmmoId;
	switch (getGrenadeIdentify(entity))
	{
		case GRENADE_EXPLOSIVE:
		{
			nadeAmmoId = pev(entity, PEV_NADEAMMOID)
			if (nadeAmmoId == g_nade_infect)
			{
				infection_explode(entity)
				return HAM_SUPERCEDE;
			}
			else if (nadeAmmoId == g_nade_fire)
			{
				fire_explode(entity)
				return HAM_SUPERCEDE;
			}
			
			/*switch (pev(entity, PEV_NADEAMMOID))
			{
				case g_nade_infect:
				{
					infection_explode(entity)
		...
		case GRENADE_FLASHBANG:
		{
			if (pev(entity, PEV_NADEAMMOID) == g_nade_frost)
			{
				frost_explode(entity)
				return HAM_SUPERCEDE;
			}
		...
		case GRENADE_SMOKEGREN:
		{
			if (pev(entity, PEV_NADEAMMOID) == g_nade_flare)
			{
				// Get its duration
				static duration
				duration = pev(entity, PEV_FLARE_DURATION)
...
public fw_PlayerPreThink(id)
{
	...
	// --- Check if player should cache armorvalue for update ---
	if (g_updatedata[id][0] == 1)
		g_updatedata[id][3] = pev(id, pev_armorvalue)
	...
	switch (itemid)
	{
		...
		default:
		{
			...
			else // Custom additions
			{
				...
				ExecuteForward(g_fwExtraItemSelected, g_fwDummyResult, id, itemid);		//正常回傳1,用回傳ZP_PLUGIN_HANDLED阻擋

				// Item purchase blocked, restore buyer's ammo packs
				if (g_fwDummyResult >= ZP_PLUGIN_HANDLED && !ignorecost)	//(購買物品失敗(forward回傳ZP_PLUGIN_HANDLED)且非忽視花費時,回復原子彈包)
					g_ammopacks[id] += ArrayGetCell(g_extraitem_cost, itemid)
			}
		}
	}
...
{
	static team[2]		//team[0] is the only space because team[1] is marked as  an string end
...
zombieme(id, infector, nemesis, silentmode, rewards)
{
	...
	if (!silentmode)
	{
		...
		else if (fnGetZombies() == 1)
		{
			...
			g_playerscount = 0
			if (get_pcvar_float(cvar_zombiefirsthp)<=0)
				{
				 for(new i=1; i<=32; i++)
				   	if(is_user_connected(i)&& is_user_alive(i))
				 		g_playerscount ++		//依人數決定僵屍血量
				 fm_set_user_health(id, floatround((300+0.15 * float(ArrayGetCell(g_zclass_hp, g_zombieclass[id]))) * float(g_playerscount)))
				}
			else
	...
	while (file && !feof(file))
	{
		...
		switch (section)
		{
			...
			case SECTION_SOUNDS:
			{
				...
				else if (equal(key, "INFECT NOTICE"))
				{
					//Parse sounds
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
...
// Restart Round manually -since noroundend is set, so we have to restart round when any team is terminated.
restart_round()
{
	g_restartround = true
	...
	for (new id = 1; id <= g_maxplayers; id++)
	{
		...
		g_updatedata[id][0] = 1
		g_updatedata[id][1] = pev(id, pev_frags)
		g_updatedata[id][2] = cs_get_user_deaths(id)
	}
...
infection_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		// Edit by i-c0112: we comment this because the last human is now infectable
		/*if (fnGetHumans() == 1)
		{
...
fnCheckLastZombie()
{
	if (!g_restartround && g_modestarted && (fnGetHumans() == 0 || (fnGetRespawnableZombies() == 0 && !g_nemround)))
	{
		restart_round()
		return;
	}
...
fnGetRespawnableZombies()
{
	static iZombies, id
	iZombies = 0

	// Respawn Restricted?
	if (!g_nemround && !g_survround && !g_swarmround && !g_plagueround && !get_pcvar_num(cvar_allowrespawninfection))
		return 0;
	else if ((g_nemround && !get_pcvar_num(cvar_allowrespawnnem)) || (g_survround && !get_pcvar_num(cvar_allowrespawnsurv)) || (g_swarmround && !get_pcvar_num(cvar_allowrespawnswarm)) || (g_plagueround && !get_pcvar_num(cvar_allowrespawnplague)))
		return 0;

	for (id = 1; id <= g_maxplayers; id++)
	{
		if (g_zombie[id] && !g_headshot[id])
			iZombies++
	}
...
allowed_zombie(id)
{
	if ((g_zombie[id] && !g_nemesis[id]) || g_endround || !g_isalive[id] || task_exists(TASK_WELCOMEMSG) /*|| (!g_newround && !g_zombie[id] && fnGetHumans() == 1)*/) // Make last human infectable to support plugin "zp_respawn"
		return false;
...
public native_get_zombie_maxhealth(id)
{
	...
	if (g_zombie[id] && !g_nemesis[id])
	{
		...
		{
			if (get_pcvar_float(cvar_zombiefirsthp) <= 0)
				 return floatround((float(ArrayGetCell(g_zclass_hp, g_zombieclass[id])) * 0.15 + 300) * float(g_playerscount));
			else
...
// Native: zp_get_roundstart_player_count
public native_get_round_player_count()
{
	return g_playerscount;
}
